...
class BaseMover(object):
    """所有移动者的基本类"""
    #(窗口, 桌布, 東南SE座標, 窗口寬, 窗口長, 標籤,小圖寬,小圖高,真假判定)
    def __init__(self, root, canvas, position, x, y, tags, w, h, play):

        # 窗口及画布
        self.root = root
        self.canvas = canvas
        # 移动者的状态(0,1,2)分別表示存活or死亡狀態等
    ...
    def get_anchor_nw_position(self):
        ...
        #以己方飛機為例,x = 窗口480/2-圖97/2,窗高600/3*2-圖高124/2
        #己方飛機:在 窗口480/2以及,窗高600/3*2為中心點CENTER放置圖然後藉上述求該飛機圖的左上座標
    ...
    def update_life_status(self):
        ...
        if self.lives_num <= 0:
            ...
            self.state = config.life_status_rese
